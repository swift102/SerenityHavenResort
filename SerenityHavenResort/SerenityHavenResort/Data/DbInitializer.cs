    ...
    public static class DbInitializer
    {
        ...
        private static async Task SeedUserProfilesAsync(AppDbContext context, ILogger? logger = null)
        {
            ...
            try
            {
                ...
                logger?.LogInformation("Retrieved {Count} user profiles from seed data", userProfiles?.Count ?? 0);

                if (userProfiles == null || !userProfiles.Any())
                {
                    logger?.LogInformation("No user profiles to seed");
                    return;
                }
                ...
                logger?.LogInformation("Found {Count} existing user profiles", existingProfileIds.Count);

                var newProfiles = userProfiles.Where(up => !existingProfileIds.Contains(up.UserProfileID)).ToList();

                if (newProfiles.Any())
                {
                    logger?.LogInformation("Attempting to add {Count} new user profiles", newProfiles.Count);

                    // Validate that all required UserIDs exist
                    var userIds = newProfiles.Select(p => p.UserID).Distinct().ToList();
                    ...
                    {
                        logger?.LogError("Missing UserIDs for UserProfiles: {MissingIds}", string.Join(", ", missingUserIds));
                        throw new InvalidOperationException($"Cannot create UserProfiles. Missing Users with IDs: {string.Join(", ", missingUserIds)}");
                    }
                    ...
                    await strategy.ExecuteAsync(async () =>
                    {
                        ...
                        try
                        {
                            logger?.LogInformation("Enabling IDENTITY_INSERT for UserProfiles");
                            await context.Database.ExecuteSqlRawAsync("SET IDENTITY_INSERT UserProfiles ON");

                            context.UserProfiles.AddRange(newProfiles);
                            logger?.LogInformation("Added profiles to context, saving changes...");

                            await context.SaveChangesAsync();
                            logger?.LogInformation("Successfully saved user profiles");

                            logger?.LogInformation("Disabling IDENTITY_INSERT for UserProfiles");
                            await context.Database.ExecuteSqlRawAsync("SET IDENTITY_INSERT UserProfiles OFF");
                        ...
                        {
                            logger?.LogError(ex, "Error during user profile seeding transaction");
            ...
            {
                logger?.LogError(ex, "Failed to seed user profiles");
                throw;
            }
        }
