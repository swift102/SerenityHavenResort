    ...
    public class EmailSender : IEmailSender, Microsoft.AspNetCore.Identity.UI.Services.IEmailSender
    {
        ...
        public async Task SendBookingConfirmationEmailAsync(string email, Booking booking, Room room)
        {
            try
            {
                ...
                var htmlMessage = _sanitizer.Sanitize(
                    $"<h3>Booking Confirmed</h3><p>Your booking for {roomName} from {booking.CheckInDate:dd-MM-yyyy} to {booking.CheckOutDate:dd-MM-yyyy} has been confirmed.</p><p>Total Price: {booking.TotalPrice:C}</p>");
        ...
        public async Task SendBookingCancellationEmailAsync(string email, Booking booking, Room room)
        {
            try
            {
                ...
                var htmlMessage = _sanitizer.Sanitize(
                    $"<h3>Booking Cancelled</h3><p>Your booking for {roomName} from {booking.CheckInDate:dd-MM-yyyy} to {booking.CheckOutDate:dd-MM-yyyy} has been cancelled.</p>");
        ...
        public async Task SendPaymentConfirmationEmailAsync(string email, Booking booking, Room room, string paymentId)
        {
            try
            {
                ...
                var htmlMessage = _sanitizer.Sanitize(
                    ...
                    $"<p><strong>Check-In:</strong> {booking.CheckInDate:dd-MM-yyyy}</p>" +
                    $"<p><strong>Check-Out:</strong> {booking.CheckOutDate:dd-MM-yyyy}</p>" +
